...
import { FiCheck, FiChevronRight } from 'solid-icons/fi';
    ...
    return (
        <DropdownMenuPrimitive.Portal>
            <DropdownMenuPrimitive.Content
                ...
                class={cn(
                    'animate-out rounded-base border-border bg-background text-foreground ui-expanded:animate-in z-50 min-w-32 origin-[var(--kb-menu-content-transform-origin)] overflow-hidden border-2 p-1 shadow-md',
                    props.class
                )}
                {...rest}
            />
...
const DropdownMenuItem = <T extends ValidComponent = 'div'>(
    ...
    return (
        <DropdownMenuPrimitive.Item
            ...
            class={cn(
                'rounded-base hover:bg-primary hover:text-primary-foreground focus:border-border ui-disabled:pointer-events-none ui-disabled:opacity-50 relative flex cursor-default items-center gap-2 px-2 py-1.5 text-sm transition-colors outline-none select-none',
                props.class
            )}
            {...rest}
        />
...
const DropdownMenuSeparator = <T extends ValidComponent = 'hr'>(
    ...
    return (
        <DropdownMenuPrimitive.Separator
            ...
            class={cn('bg-border -mx-1 my-1 h-0.5', props.class)}
            {...rest}
        />
    );
};
...
const DropdownMenuSubTrigger = <T extends ValidComponent = 'div'>(
    ...
    return (
        <DropdownMenuPrimitive.SubTrigger
            ...
            class={cn(
                'rounded-base focus:bg-primary ui-expanded:bg-primary flex cursor-default items-center px-2 py-1.5 text-sm outline-none select-none',
                props.class
            )}
            {...rest}
        >
            {props.children}
            <FiChevronRight class="ml-auto size-4" />
        </DropdownMenuPrimitive.SubTrigger>
    );
};
...
const DropdownMenuSubContent = <T extends ValidComponent = 'div'>(
    ...
    const [, rest] = splitProps(props as DropdownMenuSubContentProps, [
        'class',
    ]);
    return (
        <DropdownMenuPrimitive.SubContent
            ...
            class={cn(
                'shadow-base animate-in bg-background text-foreground z-50 min-w-32 origin-[var(--kb-menu-content-transform-origin)] overflow-hidden rounded-md border-2 p-1',
                props.class
            )}
            {...rest}
        />
...
const DropdownMenuCheckboxItem = <T extends ValidComponent = 'div'>(
    ...
    return (
        <DropdownMenuPrimitive.CheckboxItem
            ...
            class={cn(
                'rounded-base focus:bg-primary focus:text-primary-foreground ui-disabled:pointer-events-none ui-disabled:opacity-50 relative flex cursor-default items-center py-1.5 pr-2 pl-8 text-sm transition-colors outline-none select-none',
                props.class
            )}
            {...rest}
        >
