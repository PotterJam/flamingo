...
const SliderTrack = <T extends ValidComponent = 'div'>(
    ...
    return (
        <SliderPrimitive.Track
            ...
            class={cn(
                'rounded-base border-border bg-secondary-background relative h-3 w-full grow border-2',
                local.class
            )}
            {...others}
        />
...
const SliderFill = <T extends ValidComponent = 'div'>(
    ...
    return (
        <SliderPrimitive.Fill
            ...
            class={cn('rounded-base bg-primary absolute h-full', local.class)}
            {...others}
        />
    );
};
...
const SliderThumb = <T extends ValidComponent = 'span'>(
    ...
    return (
        <SliderPrimitive.Thumb
            ...
            class={cn(
                'border-border focus-visible:ring-ring -top-1.5 block size-5 rounded-full border-2 bg-white ring-offset-white transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50',
                local.class
            )}
            {...others}
        >
