...
import { FiCheck, FiChevronsDown } from 'solid-icons/fi';
...
const SelectTrigger = <T extends ValidComponent = 'button'>(
    ...
    return (
        <SelectPrimitive.Trigger
            ...
            class={cn(
                'rounded-base border-border ring-offset-background placeholder:text-muted-foreground focus:ring-ring flex h-10 w-full items-center justify-between border-2 bg-transparent px-3 py-2 text-sm focus:ring-2 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50',
                local.class
            )}
            {...others}
        >
    ...
    return (
        <SelectPrimitive.Portal>
            <SelectPrimitive.Content
                ...
                class={cn(
                    'rounded-base bg-background text-foreground ui-expanded:animate-in ui-expanded:fade-in-0 ui-expanded:zoom-in-95 ui-closed:animate-out ui-closed:fade-out-0 ui-closed:zoom-out-95 relative z-50 min-w-32 overflow-hidden border-2',
                    local.class
                )}
                {...others}
            >
...
const SelectItem = <T extends ValidComponent = 'li'>(
    ...
    return (
        <SelectPrimitive.Item
            ...
            class={cn(
                'focus:bg-primary focus:text-primary-foreground ui-disabled:pointer-events-none ui-disabled:opacity-50 relative mt-0 flex w-full cursor-default items-center rounded-sm py-1.5 pr-8 pl-2 text-sm transition-colors outline-none select-none',
                local.class
            )}
            {...others}
        >
