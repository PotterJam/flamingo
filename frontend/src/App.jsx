...
function App() {
	...
	const [players, setPlayers] = useState([]); // {id, name, isHost, hasGuessedCorrectly}
	...
	// --- Refs ---
	// Ref for whiteboard component instance methods (like clearCanvas) - less common in React
	// We'll handle clearing via the key prop instead.
	...
	// --- Utility Functions ---
	const addChatMessage = useCallback((msgPayload) => {
	...
	const getWordBlanks = useCallback((length) => {
		if (length <= 0) return '';
	...
	const handleDraw = useCallback((drawData) => {
