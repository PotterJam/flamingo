defmodule Flamingo.RoomServerTest do
  ...
  test "first player becomes host", %{room_id: room_id} do
    {:ok, player_id, state} = RoomServer.join(room_id, "Alice")
  ...
  test "second player does not change host", %{room_id: room_id} do
    ...
    {:ok, p2, state} = RoomServer.join(room_id, "Bob")
  ...
  test "leave reassigns host when host leaves", %{room_id: room_id} do
    ...
    :ok = RoomServer.leave(room_id, p1)
  ...
  test "start_game fails if not host", %{room_id: room_id} do
    {:ok, _p1, _} = RoomServer.join(room_id, "Alice")
    {:ok, p2, _} = RoomServer.join(room_id, "Bob")
    assert {:error, :not_host} = RoomServer.start_game(room_id, p2, %{})
  end

  test "start_game fails with fewer than 2 players", %{room_id: room_id} do
    {:ok, p1, _} = RoomServer.join(room_id, "Alice")
    assert {:error, :not_enough_players} = RoomServer.start_game(room_id, p1, %{})
  end

  test "start_game fails with invalid round_count", %{room_id: room_id} do
    {:ok, p1, _} = RoomServer.join(room_id, "Alice")
    {:ok, _p2, _} = RoomServer.join(room_id, "Bob")
    assert {:error, :invalid_round_count} = RoomServer.start_game(room_id, p1, %{round_count: 0})
    assert {:error, :invalid_round_count} = RoomServer.start_game(room_id, p1, %{round_count: 6})
  end
  ...
  test "start_game fails with invalid turn_length", %{room_id: room_id} do
    ...
    assert {:error, :invalid_turn_length} =
             RoomServer.start_game(room_id, p1, %{turn_length: 14})

    assert {:error, :invalid_turn_length} =
             RoomServer.start_game(room_id, p1, %{turn_length: 121})
  end
  ...
  test "start_game accepts turn_length boundaries", %{room_id: room_id} do
    ...
    assert :ok = RoomServer.start_game(room_id, p1, %{turn_length: 15})

    {:ok, state} = RoomServer.get_state(room_id)
    assert state.turn_length == 15

    assert :ok = RoomServer.start_game(room_id, p1, %{turn_length: 120})
  ...
  test "start_game succeeds and broadcasts", %{room_id: room_id} do
    ...
    assert :ok = RoomServer.start_game(room_id, p1, %{round_count: 2, turn_length: 45})
  ...
  test "snapshot projects word choices and words for the requesting player", %{room_id: room_id} do
    {:ok, p1, _} = RoomServer.join(room_id, "Alice")
    {:ok, p2, _} = RoomServer.join(room_id, "Bob")
    {:ok, p3, _} = RoomServer.join(room_id, "Charlie")

    :ok =
      RoomServer.start_game(room_id, p1, %{
    ...
    {:ok, drawer_snapshot} = RoomServer.snapshot(room_id, drawer_id)
    {:ok, guesser_snapshot} = RoomServer.snapshot(room_id, first_guesser_id)
    ...
    {:ok, drawer_snapshot} = RoomServer.snapshot(room_id, drawer_id)
    {:ok, hidden_snapshot} = RoomServer.snapshot(room_id, second_guesser_id)
    ...
    assert :correct = RoomServer.guess(room_id, first_guesser_id, word)
    {:ok, correct_guesser_snapshot} = RoomServer.snapshot(room_id, first_guesser_id)
    {:ok, still_hidden_snapshot} = RoomServer.snapshot(room_id, second_guesser_id)
  ...
  test "start_game uses custom words exclusively", %{room_id: room_id} do
    ...
    assert :ok = RoomServer.start_game(room_id, p1, %{custom_words: custom_words})
  ...
  test "start_game rejects invalid custom words", %{room_id: room_id} do
    ...
    assert {:error, :invalid_custom_words} =
             RoomServer.start_game(room_id, p1, %{custom_words: ["cat, dog"]})

    assert {:error, :too_many_custom_words} =
             RoomServer.start_game(room_id, p1, %{custom_words: Enum.map(1..3001, &"word #{&1}")})
  end

  test "minimum turn length schedules hints before the turn ends", %{room_id: room_id} do
    {:ok, p1, _} = RoomServer.join(room_id, "Alice")
    {:ok, _p2, _} = RoomServer.join(room_id, "Bob")
    :ok = RoomServer.start_game(room_id, p1, %{turn_length: 15})
  ...
  test "rejoin succeeds for existing player", %{room_id: room_id} do
    {:ok, player_id, _} = RoomServer.join(room_id, "Alice")
    {:ok, state} = RoomServer.rejoin(room_id, player_id)
  ...
  test "leave during a game marks the player disconnected instead of removing them", %{
    ...
    :ok = RoomServer.leave(room_id, guesser)
  ...
  test "rejoin reconnects a disconnected player", %{room_id: room_id} do
    ...
    :ok = RoomServer.leave(room_id, guesser)
    {:ok, state} = RoomServer.rejoin(room_id, guesser)
  ...
  test "a reconnected player is not removed by the stale grace timer", %{room_id: room_id} do
    ...
    {:ok, _state} = RoomServer.rejoin(room_id, guesser)
  ...
  test "a disconnected guesser keeps their earned score", %{room_id: room_id} do
    ...
    :correct = RoomServer.guess(room_id, g1, word)
    :ok = RoomServer.leave(room_id, g1)
    :correct = RoomServer.guess(room_id, g2, word)

    {:ok, state} = RoomServer.get_state(room_id)
    assert state.phase == :turn_reveal
    assert Map.get(state.players, g1).score > 0

    {:ok, state} = RoomServer.rejoin(room_id, g1)
    assert Map.get(state.players, g1).score > 0
  ...
  test "next drawer skips disconnected players", %{room_id: room_id} do
    ...
    :ok = RoomServer.leave(room_id, next_in_order)
  ...
  defp start_playing(room_id, settings \\ %{round_count: 1, turn_length: 30}) do
    {:ok, p1, _} = RoomServer.join(room_id, "Alice")
    {:ok, p2, _} = RoomServer.join(room_id, "Bob")
    :ok = RoomServer.start_game(room_id, p1, settings)
  ...
  test "correct guess records and broadcasts", %{room_id: room_id} do
    ...
    assert :correct = RoomServer.guess(room_id, guesser, word)
  ...
  test "incorrect guess broadcasts", %{room_id: room_id} do
    ...
    assert :incorrect = RoomServer.guess(room_id, guesser, "wrong-answer")
  ...
  test "close guess sends private feedback instead of public chat", %{room_id: room_id} do
    ...
    assert :close = RoomServer.guess(room_id, guesser, close_guess)
  ...
  test "wrong-length guesses are normal incorrect guesses", %{room_id: room_id} do
    ...
    assert :incorrect = RoomServer.guess(room_id, guesser, wrong_length_guess)
  ...
  test "drawer cannot guess", %{room_id: room_id} do
    {_p1, _p2, word, state} = start_playing(room_id)
    assert {:error, :drawer_cannot_guess} = RoomServer.guess(room_id, state.drawer_id, word)
  end
  ...
  test "turn_reveal tracks drawn_this_round", %{room_id: room_id} do
    ...
    RoomServer.guess(room_id, guesser, word)
  ...
  test "completed turn records drawing history", %{room_id: room_id} do
    ...
    RoomServer.draw_event(room_id, drawer, start_event)
    RoomServer.draw_event(room_id, drawer, draw_event)
    :correct = RoomServer.guess(room_id, guesser, word)
  ...
  test "round increments after all players draw", %{room_id: room_id} do
    {:ok, p1, _} = RoomServer.join(room_id, "Alice")
    {:ok, p2, _} = RoomServer.join(room_id, "Bob")
    :ok = RoomServer.start_game(room_id, p1, %{round_count: 2, turn_length: 30})
  ...
  test "leave during playing reconciles turn completion", %{room_id: room_id} do
    ...
    :ok = RoomServer.leave(room_id, g2)
